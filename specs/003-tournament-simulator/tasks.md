...
- [X] T001 [P] Initialize simulation file in `src/model/simulation.py`
- [X] T002 [P] Initialize bracket file in `src/model/bracket.py`
- [X] T003 [P] Initialize CLI file in `src/cli/simulate.py`
...
- [X] T004 Implement SQLite parameter loading (teams, groups, distributions) in `src/model/simulation.py`
...
- [X] T005 [P] [US1] Define deterministic 3rd-place to knockout mapping table in `src/model/bracket.py`
- [X] T006 [US1] Implement vectorized 72-match group stage sampling logic in `src/model/simulation.py`
- [X] T007 [US1] Implement vectorized points, goal difference, and goals scored calculation in `src/model/simulation.py`
- [X] T008 [US1] Implement vectorized group sorting (np.lexsort) and identify top 2 teams per group in `src/model/simulation.py`
- [X] T009 [US1] Implement vectorized ranking of 3rd-placed teams and select the top 8 in `src/model/simulation.py`
...
- [X] T010 [US2] Implement Round of 32 assignment using the lookup table from `bracket.py` in `src/model/simulation.py`
- [X] T011 [US2] Implement stochastic 50/50 tie-breaker logic for drawn knockout matches in `src/model/simulation.py`
- [X] T012 [US2] Implement vectorized progression loops (R32, R16, QF, SF, Final) in `src/model/simulation.py`
- [X] T013 [US2] Aggregate runs into a Pandas DataFrame containing progression probabilities in `src/model/simulation.py`
...
- [X] T014 [US3] Implement vectorized HHI calculation across all simulated matches in `src/model/simulation.py`
- [X] T015 [US3] Aggregate average GCI into the final probabilities Pandas DataFrame in `src/model/simulation.py`
...
- [X] T016 Implement argparse/click CLI options (`--runs`, `--db`, `--format`, `--output`) in `src/cli/simulate.py`
- [X] T017 Integrate the simulation engine execution within `src/cli/simulate.py`
- [X] T018 Implement output formatting (table, csv, json) in `src/cli/simulate.py`
- [X] T019 Run validation against `quickstart.md` commands in terminal
